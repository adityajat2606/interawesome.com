...
function getPostTask(post: SitePost): unknown {
  return (post as { task?: unknown }).task
}
...
function DirectoryHome({
  ...
  const liveListingCount = listingPosts.length
  const featuredListings = liveListingCount ? listingPosts.slice(0, 3) : getMockPostsForTask('listing').slice(0, 3)
  const heroListings = featuredListings.slice(0, 3)
  const primaryCtaHref = '/listings'
  const primaryCtaLabel = 'Browse Business Listings'

  return (
    <main>
      <section className="bg-[linear-gradient(180deg,#ffffff_0%,#f3f4f4_55%,#eef1f2_100%)] text-[#061e29]">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href={primaryCtaHref} className="inline-flex items-center gap-2 rounded-full bg-[#1d546d] px-5 py-3 text-sm font-semibold text-[#f3f4f4] shadow-[0_10px_30px_rgba(6,30,41,0.12)] transition hover:bg-[#5f9598]">
                  {primaryCtaLabel}
      ...
      <section className="border-y border-[#061e29]/8 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="text-center">
            <h2 className="mt-4 font-display text-3xl font-semibold tracking-[-0.03em] text-[#061e29] sm:text-4xl">Featured businesses</h2>
            <div className="mx-auto mt-4 h-1 w-24 rounded-full bg-gradient-to-r from-[#1d546d] to-[#5f9598]" aria-hidden />
          </div>
